...
import {
  ...
  checkBinancePayOrderStatus,
  getBinancePayCredentials
} from './src/services/binancePayService.ts';
...
async function startServer() {
  ...
  app.post('/api/withdrawals', async (req: Request, res: Response) => {
    try {
      ...
      const feePercent = withdrawal.method === 'mpesa' 
        ...
        : Number(settings.binanceWithdrawalFeePercent || 5);
  ...
  app.post('/api/payhero/stk-push', async (req: Request, res: Response) => {
    try {
      ...
      if (!phoneNumber || typeof phoneNumber !== 'string') {
        return res.status(400).json({ error: 'Valid Kenyan Safaricom phone number is required' });
      }
      ...
      res.status(500).json({ error: err.message || 'M-Pesa STK Push initiation failed' });
    }
  });
  ...
  // ==========================================
  // BINANCE PAY CRYPTO GATEWAY
  // ==========================================

  // POST /api/binance/create-order - Create Binance Pay checkout order
  app.post('/api/binance/create-order', async (req: Request, res: Response) => {
    try {
      ...
      console.error('[API /api/binance/create-order Error]:', err.message);
      res.status(500).json({ error: err.message || 'Failed to create Binance Pay order' });
    }
  });

  // GET /api/binance/order-status/:merchantTradeNo - Check Binance Pay order status
  app.get('/api/binance/order-status/:merchantTradeNo', async (req: Request, res: Response) => {
    try {
      ...
      const status = await checkBinancePayOrderStatus(merchantTradeNo);
      res.json(status);
    } catch (err: any) {
      res.status(500).json({ error: err.message || 'Failed to check Binance Pay status' });
    }
  });

  // POST /api/binance/webhook - Webhook endpoint for Binance Pay instant IPN
  app.post('/api/binance/webhook', async (req: Request, res: Response) => {
    try {
      const payload = req.body;
      const headers = {
      ...
      const result = await handleBinancePayWebhook(payload, headers);
      res.json(result);
    } catch (err: any) {
      console.error('[API /api/binance/webhook Error]:', err.message);
      res.status(500).json({ returnCode: 'FAIL', returnMessage: err.message });
    }
  });

  // GET /api/binance/config - Binance Pay gateway configuration
  app.get('/api/binance/config', (req: Request, res: Response) => {
    const credentials = getBinancePayCredentials();
    const proto = (req.headers['x-forwarded-proto'] as string) || req.protocol || 'https';
    const host = (req.headers['x-forwarded-host'] as string) || req.get('host') || '';
    const baseUrl = host ? `${proto}://${host}` : '';
    const webhookUrl = `${baseUrl}/api/binance/webhook`;

    res.json({
      isLiveConfigured: credentials.isLive,
      mode: credentials.mode,
      merchantId: credentials.merchantId || 'RoyalService_BinanceMerchant',
      webhookUrl,
      currency: 'USDT',
      rate: '1 USDT = ~130 KES',
    });
  ...
  app.get('/api', async (req: Request, res: Response) => {
    ...
    res.json({
      ...
      endpoints: [
        ...
        { method: 'POST', path: '/api/binance/create-order', desc: 'Create Binance Pay USDT order' },
        { method: 'GET', path: '/api/binance/order-status/:merchantTradeNo', desc: 'Query Binance order status' },
        { method: 'POST', path: '/api/binance/webhook', desc: 'Binance Pay IPN webhook' },
        { method: 'GET', path: '/api/binance/config', desc: 'Binance Pay gateway status' },
      ],
    });
  });
