...
import { InvestmentPackage, PlatformSettings, WithdrawalRequest } from '../types';
      ...
      >
        ...
        <div className="relative z-10 space-y-2">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-purple-500/15 text-purple-300 text-xs font-semibold border border-purple-500/30">
            ...
            <span>Master Console & ROI Engine</span>
          </div>
          <h1 className="text-xl sm:text-2xl font-black tracking-tight text-white">
            Platform Operations & Parameter Tuning
          </h1>
                  ...
                  <tr key={w.id} className="hover:bg-[#111726]/40 transition-colors">
                    ...
                    <td className="py-3.5 px-5">
                      ...
                      <span className={`inline-flex items-center text-[10px] font-semibold px-2 py-0.5 rounded-full border ${
                        ...
                        {w.method === 'binance' ? 'Binance Pay' : 'M-Pesa B2C (Manual)'}
                      </span>
                    </td>
                    <td className="py-3.5 px-5">
      ...
      {activeAdminTab === 'settings' && (
        ...
        >
          ...
          <p className="text-xs text-slate-400 mb-6">
            Configure withdrawal threshold (KES 100 minimum), turnaround SLA, and exchange rates.
          </p>
            ...
            <div className="p-4 bg-[#0a0e17] rounded-xl border border-slate-800 space-y-4">
              <div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  />
                  <span className="text-slate-400 text-[11px]">
                    (User request: "their earnings can withdrawal whatever its above 100kenya shilling")
                  </span>
                </div>
              </div>
              ...
              <div className="pt-3 border-t border-slate-800">
                <label className="block font-semibold text-slate-300 mb-1">
                  Binance Pay Withdrawal Fee (%)
                </label>
                <div className="flex items-center gap-2">
                  <input
                    id="input-binance-fee"
                    type="number"
                    value={settingsForm.binanceWithdrawalFeePercent ?? 5}
                    onChange={(e) => setSettingsForm({ ...settingsForm, binanceWithdrawalFeePercent: Number(e.target.value) })}
                    className="w-32 px-3 py-2 bg-[#111726] border border-slate-700 rounded-xl text-xs font-bold text-white font-mono focus:outline-none focus:border-emerald-500"
                    min={0}
                    max={50}
                    step={1}
                  ...
                  />
                  <span className="text-slate-400 text-[11px]">
                    (User mandate: 5% fee applied to Binance instant payouts)
                  </span>
                </div>
              </div>
          ...
          <div className="mt-6 pt-6 border-t border-slate-800 space-y-3">
            ...
            <div className="p-3 bg-[#0a0e17] rounded-xl border border-slate-800 space-y-2.5 text-xs">
              ...
              <div className="mt-3 p-3 bg-slate-900/80 rounded-lg border border-slate-800 space-y-2">
                ...
                <p className="text-[10px] text-slate-400 leading-relaxed">
                  PayHero uses standard <strong>HTTP Basic Authentication</strong>. In your PayHero Dashboard, navigate to <strong>API Keys</strong> and generate a key. You will be issued an <strong>API Username</strong> and <strong>API Password</strong>. Configure them under <strong>Settings → Secrets</strong>:
                </p>
                <div className="bg-slate-950 p-2 rounded border border-slate-800 font-mono text-[10px] text-slate-300 space-y-1">
                  <div><span className="text-emerald-400 font-bold">PAYHERO_API_USERNAME</span>=your_payhero_username</div>
                  <div><span className="text-emerald-400 font-bold">PAYHERO_API_PASSWORD</span>=your_payhero_password</div>
                  <div className="text-slate-500 pt-0.5"># The system auto-encodes to: Buffer.from(user + ':' + pass).toString('base64')</div>
